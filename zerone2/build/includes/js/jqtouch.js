...
(function($) {
    $.jQTouch = function(options) {
        ...
        var $body,
            ...
            //tapReady=true, (already declared in index.html)
            lastTime=0,
            lastAnimationTime=0,
            touchSelectors=[],
            publicObj={},
            ...
            defaults = {
                ...
                animations: [ // highest to lowest priority
                    {selector:'.slidetopdown', name:'slidetopdown', is3d:false},
				    {selector:'.slidetopup', name:'slidetopup', is3d:false},
					...
					{selector:'.fadefast', name:'fadefast', is3d:false},
                    ...
                    {selector:'.popup', name:'popup', is3d:false},
					{selector:'.popdown', name:'popdown', is3d:false},
					{selector:'.pushup', name:'pushup', is3d:false},
					{selector:'.pushdown', name:'pushdown', is3d:false},
					...
					{selector:'.moveouttoright', name:'moveouttoright', is3d:false},
					{selector:'.moveouttoleft', name:'moveouttoleft', is3d:false},
					{selector:'.moveinfromright', name:'moveinfromright', is3d:false},
					{selector:'.moveinfromleft', name:'moveinfromleft', is3d:false},
                    {selector:'.swap', name:'swapleft', is3d:true},
					{selector:'.swapleft', name:'swapleft', is3d:true},
					{selector:'.swapright', name:'swapright', is3d:true},
					{selector:'.rtlright', name:'rtlright', is3d:true}
                ]
            };
        ...
        function doNavigation(fromPage, toPage, animation, goingBack) {
            ...
            if ($.support.animationEvents && animation && jQTSettings.useAnimations) {
                ...
                if (goingBack) {
					if (animation.name.indexOf('infromleft') > 0) {
                        finalAnimationName = animation.name.replace(/infromleft/, 'outtoleft');
                    } else if (animation.name.indexOf('outtoleft') > 0) {
                        finalAnimationName = animation.name.replace(/outtoleft/, 'infromleft'); 
					} else if (animation.name.indexOf('infromright') > 0) {
                        finalAnimationName = animation.name.replace(/infromright/, 'outtoright');
					} else if (animation.name.indexOf('outtoright') > 0) {
                        finalAnimationName = animation.name.replace(/outtoright/, 'infromright');
					} else if (animation.name.indexOf('left') > 0) {
                ...
                // Trigger animations
				//Only scroll if this is set to true in template
				if (scrolltotop=="true"){
                	scrollTo(0, 0);
				}
				...
				//added by Matthijs, if navigationEndHandler has not been called for any reason
				var repairAnimationEndDetection = window.setTimeout(navigationEndHandler,1500);
            ...
            function navigationEndHandler(event) {
				...
				//added by Matthijs, clear repair timeout
				window.clearTimeout(repairAnimationEndDetection);
                
                if ($.support.animationEvents && animation && jQTSettings.useAnimations) {
                    fromPage.unbind('webkitAnimationEnd', navigationEndHandler);
					
					//changed by Matthijs, was:  fromPage.attr('class', '');
                    if (fromPage.hasClass('keep')) {
						fromPage.attr('class', '').addClass('keep');
						} else {
						fromPage.attr('class', '');
					}
				...
				//set hash only if scrolling is set to true in template
				if (scrolltotop==true){
        ...
        function init(options) {
			...
			/* commented out by Matthijs because this is already set up in the html template(
            // Set viewport
...
*/
        ...
        function touchStartHandler(e) {
            ...
            function touchMoveHandler(e) {
				...
				//Edited by Sebastian, swipe 
				
				if (absY > absX && deltaT < 1000) {
                    if (deltaY < 0) {
                        direction = 'up';
                    } else {
                        direction = 'down';
                    ...
                    $el.unbind('touchmove',touchMoveHandler).unbind('touchend',touchEndHandler).unbind('touchcancel',touchCancelHandler);
                    $el.trigger('swipe', {direction:direction, deltaY:deltaY, deltaX: deltaX});
                }
				//end edit Sebastian
                $el.unselect();
                clearTimeout(hoverTimeout);
        ...
        $(document).ready(function() {
            ...
            $.fn.isExternalLink = function() {
                var $el = $(this);
                return ($el.attr('target') == '_blank' || $el.attr('rel') == 'external' || $el.is('area[href^="http://maps.google.com"], area[href^="mailto:"], area[href^="http://"], area[href^="tel:"], area[href^="javascript:"], area[href*="youtube.com/v"], area[href*="youtube.com/watch"],a[href^="http://maps.google.com"], a[href^="mailto:"], a[href^="http://"], a[href^="tel:"], a[href^="javascript:"], a[href*="youtube.com/v"], a[href*="youtube.com/watch"]'));
            }
